...
class OrdersController extends Controller{
	...
	//	dd($orderInfo);
        ...
        if (isset($data['name']) && $data['name'][0] != null) {
               ...
               foreach($finalOrderItemsArray as $product){
				...
				$productinfo = Product::where('id',$product['name'])->first();
				$newStock = $productinfo->product_stock;
				if($data['arrival_address_type'] =='purchase_from'){
				 $newStock = $productinfo->product_stock +  $product['product_unit'];	
				}else{
				 $newStock = $productinfo->product_stock -  $product['product_unit'];
				}
				
				Product::where('id',$product['name'])->update(['product_stock'=>$newStock]);     
   ...
   /* public function report(){
        $productItems = OrderItem::with('')->get();
        return view('admin.report',compact('productItems'));
    }*/
