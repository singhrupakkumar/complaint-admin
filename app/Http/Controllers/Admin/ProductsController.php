...
class ProductsController extends Controller{
    public function index(Request $request){
        if ($request->ajax()) {
            $data = Product::select('products.*','categories.name as cat_name')->join('categories','products.cat_id','=','categories.id')->where('product_stock','!=',0)->orderBy('products.product_stock','DESC')->get();
